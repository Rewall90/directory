...
const proposeTitle = (course, region) => {
  ...
  const gfMin = [gfWeekday, gfWeekend]
...
const proposeDesc = (course, region) => {
  ...
  if (typeof gfWeekend === "number" && gfWeekend > 0 && gfWeekend !== gfWeekday)
    gf.push(`${gfWeekend} kr helg`);
...
for (const d of drafts) {
  ...
  console.log(
    `  ${d.path.padEnd(50)}  A:${aLen}${aMark}  B:${bLen}${bMark}  meta:${d.desc.length}`,
  );
}
